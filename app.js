...
els.questionImage.addEventListener("click", () => readCurrentCardAloud());
...
function readCurrentCardAloud() {
  const card = getActiveCardMeta();
  if (!card || !window.speechSynthesis) return;

  const text = state.language === "es"
    ? (card.readAloudEs || card.readAloudEn || "")
    : (card.readAloudEn || "");

  if (!text) {
    els.cardMissingMessage.textContent = "Read-aloud coming soon for this card.";
    return;
  }
